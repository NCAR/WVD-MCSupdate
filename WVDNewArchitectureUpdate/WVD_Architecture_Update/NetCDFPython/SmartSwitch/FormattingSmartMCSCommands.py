...
#        print('17--------------------------->',int(RegMap[S1+'ADDR'+S2+CStr+'_BASE'],0))
...
def HRPrint(Command,HR,PrintHR=False):
    '''Printing commands to the smart switch with human readable information'''
    print(Command+HR if PrintHR else Command)
    
def SendRead(Conn,Command):
    CommandToSend = bytes(Command,'utf-8') + b'\r\n'
    Conn.Send(CommandToSend)
    print('Sent: ', CommandToSend, 'Response: ', Conn.Read())

def ReadWriteAllPulseDefinitions(Conn,PulseDefs,Start=0,Stop=0,Pulses2Execute=0,Read=True,HReadable=True):
